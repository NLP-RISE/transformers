...
TOKENIZER_MAPPING_NAMES = OrderedDict[str, tuple[Optional[str], Optional[str]]](
    [
        ...
        (
            "flex_olmo",
            (None, "GPT2TokenizerFast" if is_tokenizers_available() else None),
        ),
        (
            "fnet",
            (
        ...
        (
            ...
            (
                ...
                (
                    "LlamaTokenizerFast"
                    if is_tokenizers_available() and not is_mistral_common_available()
                    else None
                ),
            ),
        ),
...
def tokenizer_class_from_name(class_name: str) -> Union[type[Any], None]:
    ...
    for module_name, tokenizers in TOKENIZER_MAPPING_NAMES.items():
        if class_name in tokenizers:
            ...
            if (
                module_name in ["mistral", "mixtral", "ministral"]
                and class_name == "MistralCommonTokenizer"
            ):
                module = importlib.import_module(
                    ".tokenization_mistral_common", "transformers"
                )
            else:
                module = importlib.import_module(
